...
async function fetchProgram(id: string) {
  ...
  return { id: rr.id, fields: { ...(rr.fields || {}), ...(sr.fields || {}), "Linked Actions": rr.fields?.["Linked Actions"] || [] } };
}
...
async function fetchActions(program: any) {
  const ids = program.fields?.["Linked Actions"] || [];
...
function radar(scores: any[]) {
  ...
  const avg = scores.reduce((s, x) => s + (n(x.value) ?? 0), 0) / Math.max(1, scores.length);
...
function urgent(actions: any[]) {
  const sorted = [...actions].sort((a, b) => (1 - (n(raw(b.fields || {}, ["Overall Coherence", "OCI-O"])) ?? 0)) - (1 - (n(raw(a.fields || {}, ["Overall Coherence", "OCI-O"])) ?? 0))).slice(0, 3);
...
function build(program: any, actions: any[]) {
  ...
  const scores = [
    { key: "C1", label: "Policy Coherence", value: raw(f, ["Programme C1 Claim-Evidence Score", "C1 Score", "C1 Continuity Score"]) },
    { key: "C2", label: "Operational Coordination", value: raw(f, ["Programme C2 Claim-Evidence Score", "C2 Score", "C2 Continuity Score"]) },
    { key: "C3", label: "Resource Continuity", value: raw(f, ["Programme C3 Claim-Evidence Score", "C3 Score", "C3 Continuity Score"]) },
    { key: "C4", label: "Monitoring Reliability", value: raw(f, ["Programme C4 Claim-Evidence Score", "C4 Score", "C4 Continuity Score"]) },
    { key: "C5", label: "Escalation Readiness", value: raw(f, ["Programme C5 Claim-Evidence Score", "C5 Score", "C5 Continuity Score"]) },
    { key: "C6", label: "Auditability Integrity", value: raw(f, ["Programme C6 Claim-Evidence Score", "C6 Score", "C6 Continuity Score"]) }
  ];
  const governanceScore = raw(f, ["Final Programme Coherence Score", "Program Governance Score", "Program Coherence Score", "Overall Coherence Score"]);
  const continuityScore = raw(f, ["Final Programme OCI-O Score", "Program Continuity Score", "Programme Stability Index"]);
  const stability = raw(f, ["Final Programme OCI-D Score", "Action Aggregation Coherence Score", "Programme Stability Index"]);
  ...
  return {
    ...
    contradictions: field(f, ["Cross-Action Contradictions", "Cross-Action Contradictions (AI)"], "No direct contradictions detected."),
    reviewerAction: field(f, ["Reviewer Action Required", "Recommended Reviewer Focus"], "Review programme governance condition."),
    recommendedFocus: field(f, "Recommended Reviewer Focus", "Validate monitoring, escalation and continuity logic."),
...
app.get("/api", async (req, res) => {
  try {
    ...
    if (!id) return res.type("html").send(page(build({ id: "Demo", fields: {} }, [])));
